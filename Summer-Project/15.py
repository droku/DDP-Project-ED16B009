...
train_df= pd.read_csv('t15_aug.csv',index_col=None, header=0)
valid_df= pd.read_csv('v15_aug.csv',index_col=None, header=0)
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_loss < best_loss:
                ...
                best_model_wts_loss = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), "saved_plts_wts/15/loss.pt")
            if phase == 'val' and epoch_roc_score > best_roc_score:
                best_roc_score = epoch_roc_score
                best_model_wts_acc = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), "saved_plts_wts/15/roc.pt")
    ...
    df.to_csv('saved_plts_wts/15/train_loss.csv', index=False)
    df = pd.DataFrame(vl_ls, columns = ['val_loss'])  
    df.to_csv('saved_plts_wts/15/val_loss.csv', index=False)
    df = pd.DataFrame(vl_acc, columns = ['val_roc'])  
    df.to_csv('saved_plts_wts/15/val_roc.csv', index=False)
    ...
    plt.savefig('saved_plts_wts/15/LossEpoch.png')
    plt.close()
    plt.figure()
    ...
    plt.savefig('saved_plts_wts/15/AccuracyEpoch.png')
    plt.close()
...
# size = 512
model_ft.load_state_dict(torch.load("saved_plts_wts/15/loss.pt"))
...
for c in col[2:]:
    df[c] = 0
df.to_csv('saved_plts_wts/15/mirl_results_loss.csv', index=False)
print("LOSS DONE")

model_ft.load_state_dict(torch.load("saved_plts_wts/15/roc.pt"))
...
for c in col[2:]:
    df[c] = 0
df.to_csv('saved_plts_wts/15/mirl_results_roc.csv', index=False)